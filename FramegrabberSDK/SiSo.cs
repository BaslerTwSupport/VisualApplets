...
namespace FramegrabberSDK
{
    public class SiSo
    {
        ...
        {
            _fameId = SiSoCsRt.Fg_getLastPicNumberBlockingEx(_fg, _fameId + 1, i, 5, _pmem);
            if (_fameId < 0)
            {
                throw new Exception($"Error in Fg_getLastPicNumberBlockingEx(): {SiSoCsRt.Fg_getLastErrorDescription(_fg)} ({_fameId})");
            }
            var img = SiSoCsRt.Fg_getImagePtrEx(_fg, _fameId, i, _pmem);
            if (img == null)
            {
                StopAcquireEx();
                DisconnectCamera();
            ...
            return img.toByteArray(Convert.ToUInt32(_bufferSize));
        }
